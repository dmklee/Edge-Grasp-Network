...
def render_images(sim: ClutterRemovalSim, n: int, rng: np.random.RandomState):
    ...
    origin = Transform(
        Rotation.identity(), np.r_[sim.size / 2, sim.size / 2, 0.0 + 0.25]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dest", type=str, default="./grasp_data", help="Folder where data is saved"
    )
    parser.add_argument(
        "--num_grasps",
        type=int,
    ...
    parser.add_argument(
        "--scene", type=str, choices=["pile", "packed"], default="packed"
    )
    parser.add_argument(
        "--object_set",
        type=str,
